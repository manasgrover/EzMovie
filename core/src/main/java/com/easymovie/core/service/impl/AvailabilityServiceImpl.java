...
public class AvailabilityServiceImpl implements AvailabiltyService{
	...
	public SlotAvailaibilityResponse getSlotAvailability(SlotAvailaibilityRequest availabiltyRequest) {
		List<Availabilty> availability = availabilityHandler.getTimeSlotScheduleByDateAndTheatreNameAndMovieName(availabiltyRequest.getTheatreName(),availabiltyRequest.getDate().getTime(),availabiltyRequest.getMovie());
